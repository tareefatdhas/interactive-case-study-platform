...
export function triggerStudentHaptic(tone: StudentHapticTone) {
  ...
  // Safari only produces haptics when a person directly taps a native switch.
  // A hidden programmatic switch does not provide dependable physical feedback,
  // so do not add a misleading DOM control when the vibration API is absent.
